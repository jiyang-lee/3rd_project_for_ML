...
def priority_tier(risk_probability: float, score: float) -> str:
    if risk_probability < PRE_EVENT_THRESHOLD or score < 50:
        return "monitor"
    if score >= 80:
        return "high"
    if score >= 65:
...
@dataclass
class GroupPolicy:
    """고장군별 group_weight / leadtime 프로파일 (07_데이터산출물 CSV 기반)."""
